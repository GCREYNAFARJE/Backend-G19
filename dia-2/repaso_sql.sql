...
('Rodrigo Juarez Quispe', 'rjuarez@gmail.com', 'TIPO:_A', true),
('Mariana Sanchez Gil', 'msanchez@hotmail.com', 'TIPO_B', true),
('Julia Taco Martinez', 'jtaco@gmail.com', 'TIPO:_A', true),
('Gabriel Gonza Perez', 'ggonza@yahoo.es', 'TIPO_C', false);

-- FUNCIONES DE AGREGACION (nos puede ayudar a mejorar el codigo)
-- promedio > avg()
-- minimo > min()
-- maximo > max(COLUMNA_NUMERICA)
-- contar > count(COLIMNA O REGISTRO CUALQUIERA)
...
-- Cuantos clientes son del TIPO:_A o TIPO_B
-- 3 TIPO:_A
-- 2 TIPO_B

SELECT COUNT(*), status FROM clientes 
WHERE status = 'TIPO:_A' OR status = 'TIPO_B' 
GROUP BY status
ORDER BY count(*) ASC;
